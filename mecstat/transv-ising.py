...
def ek(k, h):
    return 2*h - 2*J*np.cos(k*a)
    #return 2*np.sqrt(J**2 + h**2 - 2*J*h*np.cos(k*a))

def main():
    kmin = -np.pi/a; kmax = np.pi/a
    ks = np.linspace(kmin, kmax, 400)
    plt.plot(ks, ek(ks, 2.00), label=r'$h/J = 2.00$')
    plt.plot(ks, ek(ks, 1.50), label=r'$h/J = 1.50$')
    plt.plot(ks, ek(ks, 1.25), label=r'$h/J = 1.25$')
    plt.plot(ks, ek(ks, 1.10), label=r'$h/J = 1.10$')
    plt.plot(ks, ek(ks, 1.00), label=r'$h/J = 1.00$')
