...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\columnwidth]{fig/fig2-abcd-eps-converted-to.pdf}
\includegraphics[width=0.42\columnwidth]{fig/fig2-ef-eps-converted-to.pdf}
...
\subsection{Geometric description} \label{sec:tbg_geom}

The TBG system can be characterized by a rotation angle $\theta$ from the initial AB stacking stable setting. The two slightly misaligned layers form an interference Moiré pattern, which can be commensurate (form a superlattice) or not. In the commensurate case, one can define primitive lattice vectors $\vb{L}_1$, $\vb{L}_2$ as the least common multiple of the unit vectors of two layers. Figure \ref{fig:latvec} shows an example of this.
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{fig:bzminibz}
\end{subfigure}
\caption{(a) Commensurate case and lattice vectors $\vb{L}_1$, $\vb{L}_2$. (b) Monolayers (red and green) and mini BZ's with $(m,n) = (1,2)$ and $\theta = 21.8^\circ$. Figures taken from \cite{koshino2012}.}
\label{fig:geometry}
\end{figure}
...
The superlattice vectors $\vb{L}_1$, $\vb{L}_2$ are related by a $60^\circ$ rotation. In general, because $\vb{L}_1$ belongs to the lattices of both layers, it is written by integers $m,n,m',n'$ as
\begin{equation} \label{eq:L1}
\vb{L}_1 = m\vb{a}_1^{(1)} + n\vb{a}_2^{(1)} = m'\vb{a}_1^{(2)} + n'\vb{a}_2^{(2)}.
\end{equation}

There is an appropriate choice of lattice vectors $\vb{a}_1^{(1)}, \vb{a}_2^{(1)}, \vb{a}_1^{(2)}, \vb{a}_2^{(2)}$ such that the indices $(m',n')$ can be made equal to $(n,m)$ \cite{koshino2012}. With this in mind, one obtains the condition of commensurability for the twist angle
...
The area of an unit cell of the superlattice is $A = \frac{\sqrt{3}}{2} L^2 = (m^2 + n^2 + mn) \, A_1$, where $A_1 = \frac{\sqrt{3}}{2} \, a^2$ is the area of unit cell of monolayer graphene. Considering the BZ, we get
...
\end{equation}
where $\alpha = A,B$ indexes the type of the site, and $\tau_{\ell,\alpha}$ are basis vectors. The lattice vectors are the rotated $\a_{\ell,j} = R_{\theta_\ell} \a_{j}$, where
...
\end{pmatrix},
\end{equation}
is the rotation matrix by an angle $\theta$ and we use $\theta_1 = -\theta/2$ and $\theta_2 = +\theta/2$. We shall consider AB-stacking as our starting position, because this is the most stable configuration for bilayer graphene. In this case, the translation vectors are given by
\begin{align}
...
Here we will review the Bistritzer-MacDonald (BM) model \cite{macdonald2011}. Defining the wave vectors $\vb{G}_{\ell,1} = \vb{b}_{\ell,1}$, $\vb{G}_{\ell,2} = \vb{b}_{\ell,2}$ and $\vb{G}_{\ell,3} = \vb{b}_{\ell,1} - \vb{b}_{\ell,2}$, the moiré pattern can be interpreted by an beat effect with periodicity $\vb{b}_k^{\text{m}} = \vb{G}_{1,k} - \vb{G}_{2,k}$, \cite{handbook2019}:
...
The starting point is a tight-binding model $ H = H_1 + H_2 + H_{\perp} $, where $H_\ell$ is the hamiltonian of the layer $\ell$, and $H_\perp = V_{12} + V_{12}^\d$ describes the interlayer hybridization. We write this hamiltonian in terms of Bloch waves
...
Now, the tight-binding interlayer Hamiltonian in second quantization reads
...
Assuming the interlayer hopping $t_{12}^{\alpha\beta}(\R_1,\R_2)$ is only a function of the separation between the centers of the two orbitals, its Fourier transform is
...
Equation \ref{eq:interlayer-hopping-simplified} depends on the functional form of $t_{12}^{\alpha\beta}(\p)$. \ALERT{We now make further assumptions to make progress}. Since both $A$ and $B$ sites of graphene correspond to a $p_z$ orbital of carbos, we assume that $t_{12}^{\alpha\beta}(\r) = t_\perp(\r)$ does not depend on $\alpha,\beta$. In \cite{tperp-laissardiere2012}, the authors model the behavior of $t_\perp(\r)$ by employing Slater-Koster parameters, and numerically evaluate $t_\perp(\p)$ through equation \ref{eq:interlayer-hopping-fourier}. They conclude that $t_\perp(\p)$ is only a function of $\abs{\p}$ and decays very rapidly. This has important consequences, because only few umklapp processes with contribute to the coupling.

%\subsection{\textcolor{red}{Small rotation limit}}

In the small angle limit $\theta \lesssim 10^\circ$, we can expand around the Dirac points of each layer $\k_\ell = \K_\ell + \q_\ell$ with $\abs{\q_\ell} \sim \abs{\Delta \K} \ll \abs{\K}$. Approximating $t_\perp(\K_1+\q_1+\G_1) \approx t_\perp(\K_1+\G_1)$, the interlayer coupling becomes
$$
...
As $t_\perp(\p)$ decays rapidly with $\abs{\p}$, we truncate $t_\perp(\p) \approx 0$ for $\abs{\p} > \abs{\K}$. This leaves us only three options for $\G_\ell$, being $\g_{\ell 1} = \0$, $\g_{\ell 2} = \b_{\ell 2}$ or $\g_{\ell 3} = -\b_{\ell 1}$. They correspond to the three equivalent Dirac points on the monolayer BZ. Thus, the only quantity that matters is $t_\perp(\abs{\K})$. We have the term
$$
